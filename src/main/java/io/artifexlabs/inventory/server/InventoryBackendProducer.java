...
/**
 * Selects the storage backend the bus workers act through. {@code inventory.storage=memory} (default) serves everything
 * from memory; {@code inventory.storage=pg} uses the Postgres implementations against the configured reactive
 * datasource. The Pool is resolved lazily so memory mode needs no datasource at all.
 *
 * Since the event-bus migration this producer lives in inventory-server: the domain beans (and their transactions and
 * audit writes) belong to the worker host, not the HTTP gateway. The gateway keeps its own copy solely for its embedded
 * single-process mode.
 *
 * On startup the configured admin user is ensured to exist (idempotent), and in memory mode the configured static API
 * token is seeded so dev and test flows work without logging in first.
 */
@ApplicationScoped
public class InventoryBackendProducer {
  ...
  void onStart(@Observes StartupEvent ev, UserStore users, TokenService tokens) {
    InventoryUser admin = users.ensureUser(config("inventory.admin.email", "admin@example.com"), "Administrator",
        config("inventory.admin.password", "change-me"), true).toCompletableFuture().join();
